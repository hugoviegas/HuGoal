...
const sizeMap = {
  sm: { track: "w-10 h-6", thumb: "h-5 w-5", thumbSize: 20, travel: 18 },
  md: { track: "w-12 h-7", thumb: "h-6 w-6", thumbSize: 24, travel: 22 },
  lg: { track: "w-14 h-8", thumb: "h-7 w-7", thumbSize: 28, travel: 26 },
};
...
export function Toggle({
  ...
  const translateX = useRef(
    new Animated.Value(checked ? sizeMap[size].travel : 0),
  ).current;
  ...
  return (
    ...
    >
      <View
        ...
        style={{ borderColor: palette, borderWidth: checked ? 0 : 1 }}
      />
      <Animated.View
        style={[
